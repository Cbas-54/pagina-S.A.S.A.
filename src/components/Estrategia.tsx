...
// --- HOISTING estático decoraciones ---
...
const HeaderEstrategia = React.memo(() => (
...
HeaderEstrategia.displayName = "HeaderEstrategia";

// --- MEMOIZATION para Steps ---
interface StepCardProps {
  step: typeof STEPS_DATA[0];
  index: number;
}
const StepCard = React.memo(({ step, index }: StepCardProps) => {
  return (
    <motion.div
...
// Ojo: Esto recibe props dinámicos, no memoizamos tan fácil
const AnimatedPath = ({ pathLength }: { pathLength: any }) => (
...
const Estrategia = React.memo(() => {
  ...
  return (
    <section 
      id="procesos" 
      ref={containerRef} 
      className="relative py-40 px-6 overflow-hidden bg-bg-alt/30"
